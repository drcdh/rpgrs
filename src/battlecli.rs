...
impl<R: Iterator<Item=Result<Key, std::io::Error>>, W: Write> BattleUI for
BattleCLI<R, W> {
...
BattleCLI<R, W> {
    ...
    fn write_menus(&mut self, battle: &Battle) {
        if battle.get_top_menu_options().is_some() { // fixme
            let (menus, selections) = battle.get_menu_selections();
            if selections.is_empty() { return; }
            let depth = if battle.targets.is_empty() { selections.len() } else { selections.len() + 1 };
    ...
    fn _draw_boxes(&mut self, n: usize, baddies: bool, battle: &Battle) {
        ...
        for i in 0..n {
            ...
            let mut row = if battle.targets.contains(&pi) { String::from(TARGET_OUTER_ROW) } else if battle.is_pc_turn(&pi) { String::from(TURN_OUTER_ROW) } else { String::from(OUTER_ROW) };
        ...
        for _ in 2..=BOX_HEIGHT {
            for i in 0..n {
                let pi = if baddies { PlayerIndex::Baddy(i) } else { PlayerIndex::Ally(i) };
                let mut row = if battle.targets.contains(&pi) { String::from(TARGET_INNER_ROW) } else if battle.is_pc_turn(&pi) { String::from(TURN_INNER_ROW) } else { String::from(INNER_ROW) };
                if battle.is_player_down(&pi) {
                    row = format!("{}{}{}", color::Fg(color::Red), row, color::Fg(color::Reset));
                }
                write!(self.stdout, "{}", row).unwrap();
